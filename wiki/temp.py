...
df = pd.read_csv("graph/graph1.csv")
...
edges = [
    (name_to_index[src], name_to_index[dst]) for src, dst in zip(df["From"], df["To"])
]
print(4)
...
connectednodes = g.neighborhood("France", mode="out")
...
node_trace.marker.size = [10 + 20 * (deg / max_degree) for deg in node_adjacencies]
